...
SymbolRate = 100e6;                 % Targeted Symbol Rate
NumOfSamplesPerSymbol = 20;        % Adding Zeros Before RRC Filter (5 or 8 gives good result;28;40)
...
Tx = TxSignal_temp2*25;     % Amplify 25 times!
...
FrequencyPortionRemainAfterLED = (100e6)/(SamplingFrequency/2);
...
%fvtool(flatLowpass);
Tx_passingLED = filter(flatLowpass.Numerator,1,Tx);  
%Tx_passingLED = Tx;

%% Passing Channel in room
load('IR_FL_55Point.mat');
Tx_passedChannel_temp = conv(impulse_response_FL,Tx_passingLED);
...
% Tx_passedChannel = Tx_passingLED;
...
RxSig_I = RxSig_I((length(InFilter)-1)/2+4:end+3-(length(InFilter)-1)/2);   %good sampling phase shift
RxSig_Q = RxSig_Q((length(QnFilter)-1)/2+4:end+3-(length(QnFilter)-1)/2);
...
title("Downsampled Signal Contellation with Symbol Rate 100 MS/s");
